...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.modules.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg transition duration-150 ease-in-out flex items-center">
                        ...
                        <div class="flex items-center space-x-2" onclick="event.stopPropagation()">
                            ...
                            <form action="{{ route('admin.modules.destroy', $module) }}"
                                ...
                                <button type="submit"
                                        ...
                                        {{ $module->children->count() > 0 || $module->permissions->count() > 0 ? 'disabled' : '' }}>
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div class="bg-white p-3 rounded-lg shadow-sm">
                                <p class="text-sm text-gray-600 mb-1">Ruta</p>
                                <p class="font-mono text-sm">{{ $module->route ?: 'No definida' }}</p>
                            </div>
                            <div class="bg-white p-3 rounded-lg shadow-sm">
                                <p class="text-sm text-gray-600 mb-1">Orden</p>
                                <p class="font-mono text-sm">{{ $module->order_position }}</p>
                            </div>
                            <div class="bg-white p-3 rounded-lg shadow-sm">
                                <p class="text-sm text-gray-600 mb-1">Icono</p>
                                <p class="font-mono text-sm"><i class="fas {{ $module->icon ?: 'fa-cube' }} mr-2"></i>{{ $module->icon ?: 'fa-cube' }}</p>
                            </div>
                            @if($module->description)
                            <div class="md:col-span-3 bg-white p-3 rounded-lg shadow-sm">
                        ...
                        <div class="mb-4">
                            <h4 class="font-medium text-gray-700 mb-2 flex items-center">
                                ...
                                Permisos ({{ $module->permissions->count() }})
                            </h4>
                            <div class="flex flex-wrap gap-2">
                                ...
                                @empty
                                    <p class="text-sm text-gray-500">No hay permisos definidos</p>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div>
                            <h4 class="font-medium text-gray-700 mb-2 flex items-center">
                                ...
                                Submódulos ({{ $module->children->count() }})
                            </h4>
                            <div class="space-y-2">
                                @foreach($module->children as $submodule)
                                    <div class="flex justify-between items-center p-3 bg-white rounded-lg shadow-sm">
                                        <div class="flex items-center space-x-3">
                                            <i class="fas {{ $submodule->icon ?: 'fa-folder' }} text-gray-500"></i>
                                            <span class="font-medium">{{ $submodule->name }}</span>
                                            <span class="text-sm text-gray-500">{{ $submodule->route }}</span>
                                        </div>
                                        <a href="{{ route('admin.modules.edit', $submodule) }}"
