...
<div class="bg-white rounded-lg shadow-lg">
    <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800">Editar Moneda: {{ $moneda->nombre }}</h3>
    </div>

    <form action="{{ route('admin.moneda.update', $moneda->id) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text"
                       name="pais_code"
                       id="pais_code"
                       value="{{ old('pais_code', $moneda->pais_code) }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div>
                ...
                <input type="number"
                       ...
                       id="tasa_cambio_usd"
                       value="{{ old('tasa_cambio_usd', $moneda->tasa_cambio_usd) }}"
                       required
                       step="0.01"
                       min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div class="flex items-center space-x-6">
                    <label class="inline-flex items-center">
                        <input type="radio"
                               name="is_active"
                               value="1"
                               {{ old('is_active', $moneda->is_active) == '1' ? 'checked' : '' }}
                    ...
                    <label class="inline-flex items-center">
                        <input type="radio"
                               name="is_active"
                               value="0"
                               {{ old('is_active', $moneda->is_active) == '0' ? 'checked' : '' }}
                               class="form-radio text-indigo-600 focus:ring-indigo-500">
                ...
                <div class="flex items-center">
                    <label class="inline-flex items-center">
                        <input type="checkbox"
                               name="aceptar_pagos"
                               value="1"
                               {{ old('aceptar_pagos', $moneda->aceptar_pagos) ? 'checked' : '' }}
